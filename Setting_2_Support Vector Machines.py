...
X = df.drop(['domain', 'classification', 'length', 'vowel_count'], axis=1)
y = df['classification']
...
print(f"True Negatives: {TN}")# Get feature importances
...
for feature, coef in zip(X.columns, normalized_coefficients):
    print(f"Feature: {feature}, Normalized Coefficient: {coef}")
...
results = {
    ...
    'False Negatives': FN,
    'True Negatives': TN,
**dict(zip(X.columns, normalized_coefficients))
}
...
results_df.to_csv('evaluation_results_Setting2_supportvectormachines.csv', index=False)
